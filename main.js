...
let apiKey = "eda00bd1c3b6deec2e3cc211";
...
//   console.log(fromCurrrencyCode); -->> just testing
//   console.log(toCurrencyCode); -->> just testing
  ...
  if (amount !== "" && parseFloat(amount) >= 1) {
    ...
    fetch(url)
      .then((resp) => resp.json())
      .then((data) => {
        // console.log(data.conversion_rate); -->> just testing

        const conversionResult = (amount * data.conversion_rate).toFixed(2);
       
        result.innerHTML = `${amount} ${fromCurrrencyCode} = ${conversionResult} ${toCurrencyCode}`;
